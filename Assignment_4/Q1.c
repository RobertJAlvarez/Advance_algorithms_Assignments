...
static void update_size(size_t *size)
{
  if (*size < 10) *size += ((size_t) 1);
  else if (*size < 100) *size += ((size_t) 5);
  else if (*size < 1000) *size += ((size_t) 50);
  else if (*size < 10000) *size += ((size_t) 100);
  else *size += ((size_t) 1000);
}
...
int main(int argc, char **argv)
{
  ...
  size_t hash_size, max_collisions, n_entries;
  ...
  printf("hashtable_size,#_of_collisions,%%_collisions_per_entry,#_of_empty_entries\n");

  for (hash_size = ((size_t) 1); hash_size <= 100000; update_size(&hash_size)) {
    ...
    printf("242\n");
    if (n_entries == 0)
      n_entries = number_entries_in_hashtable(hashtable);

    printf("246\n");
    max_collisions = max_number_collisions_in_hashtable(hashtable);
    printf("%zu,%zu,%2.3f,%zu\n", hash_size, max_collisions, 100.0f*((float) max_collisions)/((float) n_entries), number_empty_entries_in_hashtable(hashtable));
    delete_hashtable(hashtable, delete_key, delete_value, NULL);
    break;
  }
